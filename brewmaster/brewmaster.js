...
const RAISIN_TOKEN = "0x4df22AaC6a83C44bf4eFA592E170E1a4d1dCf2c2";
// Cellar Token
...
const ENOTECA = "0x16b01aA80721ADF6209250d7F3b4575A65177Bda";
...
const updateWinery = async(account) => {
	if(usingWeb3) {
		...
		await enoteca.methods.userInfo(account).call({from: account}).then(async(userInfo) => {
			let amount = parseInt(userInfo[0]);
			enotecaBalance = (amount / 1e18);
			let debt = parseInt(userInfo[1]);
			await enoteca.methods.accRewardTokenPerShare().call({from: account}).then(async(accRewardsPerShare) => {
				await enoteca.methods.getRewardPerSecond().call({from: account}).then(async(rewardsPerSecond) => {
					await enoteca.methods.depositedBalance().call({from: account}). then(async(lpSupply) => {
						...
						console.log(accRewardToken);
					});
				});
			});
		});
		
		await enoteca.methods.pendingRewards(account).call({from: account}).then(function(pending) {
			enotecaPending = (pending / 1e18);
		});
	...
	if(!usingWeb3) {
		...
		await callRPC(account, ENOTECA, "userInfo(address)", [ account ]).then(async(userInfo) => {
			console.log(userInfo);
			let amount = window.web3.utils.toBN(userInfo.substring(2,66));
			enotecaBalance = (amount / 1e18);
			let debt = window.web3.utils.toBN(userInfo.substring(66,130));
			await callRPC(account, ENOTECA, "accRewardTokenPerShare()", [ ]).then(async(accRewardsPerShare) => {
				
				await callRPC(account, ENOTECA, "getRewardPerSecond()", [ ]).then(async(rewardsPerSecond) => {
					
					await callRPC(account, ENOTECA, "depositedBalance()", [ ]).then(async(lpSupply) => {
		...
		await callRPC(account, ENOTECA, "pendingRewards(address)", [ account ]).then(function(pending) {
			enotecaPending = (pending / 1e18);
		});
	}
	...
	$("#resetCost").html(resetCost.toFixed(2) + " RAISIN<br>" + ((parseInt(normalCount) + parseInt(masterCount) + parseInt(toolCount)) * 15) + ".00 VINTAGE");
	...
	if(usingWeb3) {		
		...
		await grape_nodes.methods.users(account).call({from: account}).then(async(userInfo) => {
			...
			await grape_nodes.methods.getDayDripEstimate(account).call({from: account}).then(async(dripEstimate) => {
					...
					gns.apr = ((gns.dripRate/1e18) / ((userInfo['nodes']-userInfo['compounds'])*50))*100
	...
	if(!usingWeb3) {
		...
		await callRPC(account, GRAPE_NODES, "users(address)", [ account ]).then(async(userInfo) => {
			...
			await callRPC(account, GRAPE_NODES, "getDayDripEstimate(address)", [ account ]).then(async(dripEstimate) => {			
					gns.dripRate = dripEstimate;
					gns.apr = ((gns.dripRate / 1e18) / ((gns.count - gns.compounds)*50))*100	
...
const web3Query = async () => {
	...
	if(await Web3Enabled()) { // Initialize web3 and connected to metamask
		...
		window.web3.eth.getAccounts().then(function(accounts) {
			...
			grape = new window.web3.eth.Contract(ERC20_ABI, GRAPE_TOKEN);
			vint = new window.web3.eth.Contract(ERC20_ABI, VINT_TOKEN);
			raisin = new window.web3.eth.Contract(ERC20_ABI, RAISIN_TOKEN)
			mim = new window.web3.eth.Contract(ERC20_ABI, MIM_TOKEN);
			wine = new window.web3.eth.Contract(ERC20_ABI, WINE_TOKEN);
			...
			enoteca = new window.web3.eth.Contract(ENOTECA_ABI, ENOTECA);
...
const resetFatigue = async() => {	
	window.web3.eth.getAccounts().then(async(accounts) => {
		...
		winery.methods.getTotalPPM(account).call({from: account}).then(async(totalPPM) => {
			...
			await raisin.methods.balanceOf(account).call({from: account}).then(async(balance) => {
				console.log("Balance   : " + balance);
				await raisin.methods.allowance(account, WINEMAKER_REWARDS).call({from: account }).then(async(allowance) => {
					if(window.web3.utils.toBN(balance).lt(window.web3.utils.toBN(reset_cost))) {
						console.error("[ERROR] Insufficient RAISIN balance to reset, " + (balance/1e18).toFixed(0) + "/" + (reset_cost/1e18).toFixed(0));
						return;
					}

					console.log("Allowance: " + (allowance));
					if(window.web3.utils.toBN(allowance).lt(window.web3.utils.toBN(totalPPM * RESET_MULTI))) {
						await raisin.methods.approve(WINEMAKER_REWARDS, MAX_UINT256).send({from: account}).on("confirmation", function(confirms) {
							if(confirms === 5) {
...
const claimRaisin = async() => {	
	window.web3.eth.getAccounts().then(async(accounts) => {
		let account = accounts[0];
		enoteca.methods.withdraw(0).send({from: account});
	});
};
